...
(motivation for the package on having tools for diagnosting mixed models) The Merriam-Webster dictionary defines [redress](https://www.merriam-webster.com/dictionary/redress) as "to set right".
...
paprika_plus <- cbind(paprika, rcr, pmr, scr)
head(paprika_plus)
```
...
```{r}
# Creates a shiny app with only one model
redres_app(model = m)
```
