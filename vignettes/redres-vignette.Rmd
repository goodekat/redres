...
`redres` is structured so the package functions fall under one of three categories. These categories are divided based on the type of model assessment experience the functions provide.

1. [Residual extraction](#compute_redres): The first manner in which `redres` can be used is to obtain residuals from a model. This can be done using the main function in the package named [**`compute_redres`**](#compute_redres) to compute and return a vector of residuals of a specified type. The user can then make use of the residuals to perform their own methods of assessment.

\vspace{0.25cm}

2. [Diagnostic plot creation](#plots): The second way to utilize `redres` is to easily create diagnostic plots from a model with one of the following functions.
    - [**`plot_redres`**](#plot_redres): Creates a plot of residuals versus fitted values or a model variable for a given residual type
...
3. [Interactive model assessment](#app): Lastly, `redres` can be employed to facilitate an interactive model assessment experience. The function [**`launch_redres`**](#launch_redres) accepts one or two models and opens a Shiny app package provided diagnostic tools. Within the Shiny window, the user can view and interact with all the plots available in the `redres` package. If two models are input to the function, plots from both models are shown side by side to help with model selection.

This vignette is set up to mimic the package structure. The sections after the introduction are dedicated to one of the three package uses. These sections contain additional details on the functions (such as the available [available types](#types)), explanations of how to interpret output, and example applications of the functions.
...
(1) independence of our observations, (2) a linear relationship between the response and the explanatory variables, (3) constant variance and (4) normality of the error term.
...
One of the ways to utilize `redres` is to extract residuals from a model fit using `lmer`. This can be done using the function in the package named `compute_redres` to compute residuals based on a specified type.
...
The function `compute_redres` can compute the residuals types defined in the previous section for models fit using `lmer`.
...
- `compute_redres` returns a vector of residuals according to the type specified in the order that the data observations are input into `lmer`.

**Functionality** 

The code below demonstrates the use of `compute_redres` to compute several types of residuals from the model `m`. These residuals are put into a dataframe and joined with the `paprika` data.
...
- `model`: A model fit using `lmer` for which the residuals will be computed.
- `type`:	A character string identifying the type of residual that will be computed. The same options available with [`compute_redres`](#compute_redres) are available for `plot_redres`. By default, the conditional raw residuals are plotted.
...
<img align="center" width="600" src="figs/resid_type.png">

and x-axis variables.

<img align="center" width="600" src="figs/xvar.png">

The `Quantile Plots` tab shown below contains a random effects normal quantile plot

<img align="center" width="600" src="figs/rand_eff_qq.png">

and error term normal quantile plot.

<img align="center" width="600" src="figs/error_qq.png">
...
<img align="center" width="600" src="figs/resid_two_model.png">

And the following show one of the quantile plots:

<img align="center" width="600" src="figs/error_qq_two_mod.png">
