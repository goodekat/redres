...
When a list of two models is input into `launch_redres`, the app shows a side-by-side comparison of the plots. The user then is able to visually compare the two linear mixed models and conduct model selection.
...
Two selection widgets are included in the `Residual Plot` tab allow the user to choose the [residual types](#types) to be shown on the y-axis and the variable to be plotted x-axis variable. These can be see in the screen shot of the app below.
...
In this case, the Shiny app shows the residual plots side by side. Again, a selection is available for the residual type. For variables on the x-axis, separate selectors are included for each model to allow for cases when the two models have different variables.

<img align="center" height="250" src="figs/resid_two_model.png">

The quantile plots are also shown side by side.

<img align="center" height="250" width = "300" src="figs/error_qq_two_mod.png">
