...
<img align="right" width="150" height="175" src="figs/sticker.png">
...
`redres` contains various functions to help with model assumptions. The package is set up so that the functions fall under one of three model assessment categories. These categories distinguish between the type of model assessment experience the user would like. The three categories and their associated functions are listed below.

1. [Residual extraction](#redres)
    - [`redres`](#compute_redres): computes and returns residuals given a model and a specified residual type 
    
\vspace{0.25cm}

2. [Diagnostic plot creation](#plots)
    - [`plot_redres`](#plot_redres): This function creates a plot of residuals versus fitted values for a given residual type. The [available types](#types) are described later in the vignette.
...
3. [Interactive model assessment](#app)
    - [`redres_app`](#redres_app): This function opens an interactive Shiny app to interactively assess their model with package provided tools. Within the Shiny window, the user can view all the plots available in the `redres` package but with additional interactivity. Or the user can input two models into the app function for model selection.

This vignette is set up to mimic the package structure. The sections after the introduction are dedicated to one of the three package uses.
